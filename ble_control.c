...
static void send_notify(void * p_context)
{
    ...
    m_char_value_notifycation[0] = rgb_color.r;
    m_char_value_notifycation[1] = rgb_color.g;
    m_char_value_notifycation[2] = rgb_color.b;
...
{   
    ...
    if (p_evt_write->handle == m_estc_service.notify_handles.cccd_handle && (p_evt_write->len == 2))
    {   
        if (ble_srv_is_notification_enabled(p_evt_write->data) == true)
        {
            ...
            APP_ERROR_CHECK(err_code);
            NRF_LOG_INFO("Start characteristic notification timer");
        }
        else
        {
            err_code = app_timer_stop(m_notify_timer_id);
            APP_ERROR_CHECK(err_code);
            NRF_LOG_INFO("Stop characteristic notification timer");
        }
    }
